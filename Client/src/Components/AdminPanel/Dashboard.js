...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Filter only for "dogs" and "cats"
                const filteredPetData = petData
                    .filter(item => item._id.toLowerCase() === 'dog' || item._id.toLowerCase() === 'cat')
                    .map(item => ({ name: item._id, value: item.count }));

                setTotalPets(filteredPetData.reduce((acc, item) => acc + item.value, 0));
                setPetData(filteredPetData);
            } catch (error) {
    ...
    const COLORS = ['#0088FE', '#FF8042']; // Assign specific colors for dogs and cats
